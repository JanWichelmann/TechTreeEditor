    ...
    public partial class ExportDATFile : Form
    {
        ...
        private void _finishButton_Click(object sender, EventArgs e)
        {
            ...
            Task.Run(() =>
            {
                ...
                foreach(var building in _projectFile.Where(b => b is TechTreeBuilding building && _projectFile.Where(el => el.BuildingDependencies.ContainsKey(building)).Count > 0))
                {
                    // Freie ID suchen
                    ...
                    // Element an direkte Upgrades und dann Zeitalter-Upgrades weitervererben
                    TechTreeBuilding currentBuildingUpgrade = (TechTreeBuilding)building;
                    while(currentBuildingUpgrade.Successor != null)
                    {
                        currentBuildingUpgrade = (TechTreeBuilding)currentBuildingUpgrade.Successor;
                        buildingDependencyResearchIDs[currentBuildingUpgrade] = lastID;
                    }
                    foreach(var au in currentBuildingUpgrade.AgeUpgrades)
