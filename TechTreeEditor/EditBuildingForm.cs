...
namespace TechTreeEditor
{
	public partial class EditBuildingForm : Form
	{
		...
		{
			// Anzahl-Spalte
			if(e.ColumnIndex == 0 && _building != null) // Verhindert, dass Ereignis schon bei Initialisierung ausgelöst wird
			{
				// Wert speichern
				_building.BuildingDependencies[(TechTreeBuilding)_buildingDepView.Rows[e.RowIndex].Tag] = (bool)_buildingDepView.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
			}
		}
